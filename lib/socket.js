...
function connectDatabase() {
	...
	MonClient.connect(createConnectionString(), function (err, db) {
		if (err) {
		...
		if (!dbCollection) {
			...
			dbDeferred.reject({
				messge: 'could not open collection %s', config.db.collection);
			})
		}
...
function setupSocket() {
	...
	dbStream.on('data', broadcastLogMessage);
...
function handleSocketConnection(socket) {
	...
	socket.on('fetchAll', handleFetchAll);	
...
function handleFetchAll(socket) {
	...
	allDocumentsDbStream.on('data', function (message) {
 ...
 */
module.export = function (pConfig, pIo, pDebug) {
	// keep references
	debug = pDebug;
	io = pIo;
	config = pConfig;
